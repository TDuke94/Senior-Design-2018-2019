...
const struct gyro_reg_s reg = {
    ...
    .fifo_count_h   = 0x72,
    .fifo_r_w       = 0x74,
    .raw_gyro       = 0x33, // updated for ICM 20948
    .raw_accel      = 0x2D, // updated for ICM 20948
    .temp           = 0x41,
    .int_enable     = 0x38,
    .dmp_int_status = 0x39,
    .int_status     = 0x3A,
    .pwr_mgmt_1     = 0x06, // updated for ICM 20948
    .pwr_mgmt_2     = 0x6C,
    .int_pin_cfg    = 0x37,
    .mem_r_w        = 0x6F,
    .accel_offs     = 0x06,
...
const struct hw_s hw = {
    .addr           = 0x68, // unchanged for icm 20948
    .max_fifo       = 1024,
    .num_reg        = 118,
    .temp_sens      = 340,
    .temp_offset    = -521,
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    data[0] = 0x00;
    if (i2c_write(st.hw->addr, 0x7F, 1, data))
        return -1;
    delay_ms(100);
...
int mpu_get_gyro_reg(short *data, unsigned long *timestamp)
{
    ...
    /* Configure for User Bank 0 */
    localData = 0x00;
    if (i2c_write(st.hw->addr, 0x7F, 1, &localData))
        return -1;
...
int mpu_set_accel_fsr(unsigned char fsr)
{
    ...
    if (st.chip_cfg.accel_fsr == (data >> 1))
        return 0;
...
int mpu_set_sample_rate(unsigned short rate)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        /* gyro */
        data = 1000 / rate - 1;
        if (i2c_write(st.hw->addr, 0x00, 1, &data))
            return -1;
        /* accel */
        if (i2c_write(st.hw->addr, 0x10, 1, &zero))
            return -1;
        if (i2c_write(st.hw->addr, 0x11, 1, &data))
            return -1;
...
int mpu_set_compass_sample_rate(unsigned short rate)
{
    ...
    if (writeMagRegister(0x031, 0x10))
        return -1;
...
int writeMagRegister(uint8_t subAddress, uint8_t data)
{
    ...
    localData = subAddress;

    // set the register to the desired magnetometer sub address
    if (i2c_write(st.hw->addr, UB3_I2C_SLV0_REG, 1, &localData))
        return -3;

    localData = data;

    // store the data for write
    if (i2c_write(st.hw->addr, UB3_I2C_SLV0_DO, 1, &localData))
        return -4;


    localData = /*UB3_I2C_SLV0_CTRL_EN | */ (uint8_t)1;

    // enable I2C and send 1 byte
    if (i2c_write(st.hw->addr, UB3_I2C_SLV0_CTRL, 1, &localData))
        return -5;
